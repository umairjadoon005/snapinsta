...
function triggerDownload(videoUrl: string) {
  ...
  const filename = `snapinsta-ink-${randomTime}.jpg`;
...
export function InstagramFormImage(props: { className?: string }) {
  ...
  return (
    <div className={cn("w-full space-y-5", props.className)}>
      {errorMessage ? (
        <p className="h-4 text-sm text-red-300 sm:text-start">{errorMessage}</p>
      ) : (
      ...
      <p className="text-muted-foreground text-white text-center text-xs">{t("hint")}</p>
    </div>
  );
}
